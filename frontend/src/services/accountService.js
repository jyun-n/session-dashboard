...
export async function createAccount(newAccount) {
  ...
  if (!res.ok) {
    return { success: false, message: data.message || "계정 생성에 실패했습니다." };
  }

  return { success: true, message: "계정이 생성되었습니다.", account: data.data };
}
...
export async function updateAccount(accountId, { dept, position }) {
  ...
  if (!res.ok) {
    return { success: false, message: data.message || "회원 정보 수정에 실패했습니다." };
  }

  return { success: true, message: "회원 정보가 수정되었습니다.", account: data.data };
}

export async function getAllLoginLogs() {
  const res = await fetch(`${API_URL}/accounts/logs/all`, {
    headers: getHeaders(),
  });

  if (res.status === 401) {
    handleUnauthorized();
    return { success: false, message: "인증이 만료되었습니다.", logs: [] };
  }

  const data = await res.json();

  if (!res.ok) {
    return { success: false, message: data.message || "로그를 불러오지 못했습니다.", logs: [] };
  }
...
export async function deleteAccount(accountId, secret) {
  ...
  if (!res.ok) {
    return { success: false, message: data.message || "계정 탈퇴에 실패했습니다." };
  }
